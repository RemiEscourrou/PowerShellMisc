...
<#   
   Initial code from SadProcessor
...
.EXAMPLE
   ...
   Defaults to neo4j2022
#>
Function Install-BloodHound{
    [CmdletBinding()]
    Param(
        # Specify BloodHound version
        [ValidateSet('4.0.0','4.0.1','4.0.2','4.0.3','4.1.0')]
        [Parameter(Mandatory=0)][String]$BHVersion='4.1.0',
        # Specify Neo4j Version
        [ValidateSet('4.2.11','4.1.9')]
        [Parameter(Mandatory=0)][String]$Neo4jVersion='4.2.11',
        ...
        [Parameter(Mandatory=0)][String]$quiet='neo4j2022'
        )
    ...
    if(-Not(Test-Path "C:\Program Files\Java\j*\bin\server\jvm.dll")){
        Write-Host "[!] Java not found" -ForegroundColor Red
        # Instructions
        Write-Host "[>] Go to https://www.oracle.com/java/technologies/javase-jdk11-downloads.html" -ForegroundColor Yellow
...
#####################################################EOF")
	...
	#(New-Object System.Net.WebClient).DownloadFile($Neo_Zip, "Neo4j.zip")
	...
	#(New-Object System.Net.WebClient).DownloadFile($BH_Zip, "BloodHoundMaster.zip")
    ...
    ## Set Password
    Write-Host "[I] Opening neo4j"            -ForegroundColor Green
	if($quiet){
		...
		Invoke-Neo4jAdmin set-initial-password $quiet
	}
	else{
